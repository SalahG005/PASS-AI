...
public class AiController {
    ...
    /** Scaffold/agent progress via SSE — tool steps, plan, and files stream while the run is in flight. */
    @PostMapping(value = "/agents/{mode}/stream", produces = MediaType.TEXT_EVENT_STREAM_VALUE)
    public SseEmitter specializedAgentStream(
            Authentication authentication,
            @PathVariable String mode,
            @RequestHeader(value = "X-Workspace-Id", required = false) String workspaceId,
            @RequestBody ChatRequestDto body
        ...
        streamExecutor.submit(() -> {
            try {
                AgentMode agentMode = AgentMode.fromPath(mode);
                AgentResponseDto response = aiService.specializedAgentStream(
                        user,
                        workspaceId,
                        body,
                        agentMode,
                ...
                try {
                    emitter.send(SseEmitter.event().name("error")
                            .data(Map.of("error", e.getMessage() != null ? e.getMessage() : "Ollama error")));
                } catch (IOException ignored) {
                }
                emitter.completeWithError(e);
            } catch (InterruptedException e) {
                ...
                try {
                    emitter.send(SseEmitter.event().name("error")
                            .data(Map.of("error", "Interrupted while waiting for Ollama")));
                } catch (IOException ignored) {
                }
                emitter.completeWithError(e);
            } catch (Exception e) {
                try {
                    emitter.send(SseEmitter.event().name("error")
                            .data(Map.of("error", e.getMessage() != null ? e.getMessage() : "Agent stream failed")));
                } catch (IOException ignored) {
                }
                emitter.completeWithError(e);
            }
    ...
    public ResponseEntity<?> install(
            Authentication authentication,
            @RequestHeader(value = "X-Workspace-Id", required = false) String workspaceId,
            @RequestBody InstallRequestDto body
    ) {
        try {
            InstallResultDto result = aiService.installCommands(email(authentication), workspaceId, body);
            if (!result.isOk() && result.getExecuted().isEmpty()) {
                return ResponseEntity.badRequest().body(result);
            }
            return ResponseEntity.ok(result);
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(Map.of("error", e.getMessage() != null ? e.getMessage() : "Install failed"));
        }
    }

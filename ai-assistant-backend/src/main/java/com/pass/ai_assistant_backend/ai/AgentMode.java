...
public enum AgentMode {
    CODE(
            "code",
            Set.of("read_file", "search", "write_file", "edit_file", "run_terminal", "propose_install", "done"),
            10,
            """
            ...
            When editing HTML/CSS for a user-facing page:
            - DEFAULT: Tailwind CSS via CDN in <head> + short styles.css for :root tokens (unless user asked for plain CSS only).
            - Google Font (Inter), 8/16/24/32px spacing scale, max-width container, flex/grid layout, mobile breakpoint.
            - Polished commercial look — never bare Arial + #4CAF50 defaults, no lorem ipsum walls.
            """
    ),
    REVIEW(
            "review",
    ...
    SCAFFOLD(
            "scaffold",
            Set.of("read_file", "search", "write_file", "edit_file", "submit_plan", "run_terminal", "propose_install", "done"),
            24,
            """
            ...
            FILE WORKFLOW (reduce wasted retries):
            - NEW files in the plan: write_file with action create (one complete file per call).
            - If write_file says the path ALREADY EXISTS: call read_file on that path, then write_file
              with action overwrite and the FULL new file — do NOT use edit_file for whole-file rewrites.
            - edit_file is ONLY for tiny surgical changes (exact old_string copied from read_file).
            - Package installs (npm install, pip install): use propose_install — NEVER run_terminal for installs.
            ...
            STATIC HTML/CSS DESIGN (when user wants a website / landing page):
            DEFAULT STACK: HTML + Tailwind CDN + styles.css (:root tokens) + script.js
            - Unless user explicitly asked for "plain CSS" / "no Tailwind", ALWAYS include:
                <script src="https://cdn.tailwindcss.com"></script>
              (Static asset CDNs are OK — the no-cloud rule applies to AI APIs only, not CSS/ font CDNs.)
            - styles.css FIRST in phase 1: :root palette (--color-primary, --color-bg, --color-text, --color-accent),
              spacing scale (--space-1: 8px through --space-6: 64px), Google Font on body.
            - index.html: link Google Font + Tailwind CDN + styles.css; semantic sections; realistic copy (NO lorem ipsum).
            - Layout: max-w-6xl mx-auto px-4, flex/grid (never floats), md: breakpoint minimum.
            - FORBIDDEN: default Arial + #4CAF50, unstyled browser defaults, placeholder walls.
            - submit_plan summary MUST include a "Design:" line (font, palette, Tailwind/plain, spacing scale).
            - Later phases MUST reuse the same design — read styles.css before writing new HTML.
